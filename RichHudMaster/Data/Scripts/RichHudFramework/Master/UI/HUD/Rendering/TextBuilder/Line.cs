		...
		namespace Rendering.Server
		{
			public abstract partial class TextBuilder
			{
				...
				protected abstract partial class Line : ILine
				{
					...
					/// <summary>
					/// Set true if a formatting change requires recalculating quad color
					/// </summary>
					public bool isColorStale;
					...
					public void SetFormatting(int start, int end, GlyphFormat format)
					{
						...
						for (int n = start; n <= end; n++)
						{
							...
							bool colorEqual = lastFormat.Item4 == format.Data.Item4;
							...
							{
								var fGlyph = formattedGlyphs[n];
								fGlyph.format = format;
								formattedGlyphs[n] = fGlyph;
								isColorStale = true;
							}
						}
					}
					...
					public void AddCharFromLine(int index, Line line)
					{
						...
						if (canTextBeEqual)
						{
							...
							{
								GlyphFormatMembers newFormat = line.formattedGlyphs[index].format.Data,
									lastFormat = formattedGlyphs[Count].format.Data;
								bool fontEqual = lastFormat.Item1 == newFormat.Item1
									&& lastFormat.Item2 == newFormat.Item2
									&& lastFormat.Item3 == newFormat.Item3;
								bool colorEqual = lastFormat.Item4 == newFormat.Item4;

								canTextBeEqual = fontEqual;

								// Update color if possible
								if (fontEqual && !colorEqual)
								{
									var fGlyph = formattedGlyphs[Count];
									fGlyph.format.Data = newFormat;
					...
					public void AppendRichString(RichStringMembers richString, bool allowSpecialChars)
					{
						...
						GlyphFormat newFormat = new GlyphFormat(richString.Item2);
								...
								if (text[n] >= ' ' || allowSpecialChars && (text[n] == '\n' || text[n] == '\t'))
								{
									if (canTextBeEqual)
									{
										if (chars[Count] == text[n])
										{
											GlyphFormatMembers
												lastFormat = formattedGlyphs[Count].format.Data;
											bool fontEqual = lastFormat.Item1 == newFormat.Data.Item1
												&& lastFormat.Item2 == newFormat.Data.Item2
												&& lastFormat.Item3 == newFormat.Data.Item3;
											bool colorEqual = lastFormat.Item4 == newFormat.Data.Item4;
						...
						if (srcCount > 0)
						{
							...
							if (canTextBeEqual)
							{
								for (int i = 0; i < srcCount; i++)
								{
									...
									if (canTextBeEqual)
									{
										GlyphFormatMembers newFormat = src.formattedGlyphs[k].format.Data,
											lastFormat = formattedGlyphs[j].format.Data;
										bool fontEqual =
											lastFormat.Item1 == newFormat.Item1
											&& lastFormat.Item2 == newFormat.Item2
											&& lastFormat.Item3 == newFormat.Item3;
										bool colorEqual = lastFormat.Item4 == newFormat.Item4;
										...
										if (fontEqual && !colorEqual)
										{
											var fGlyph = formattedGlyphs[j];
											fGlyph.format.Data = newFormat;
											formattedGlyphs[j] = fGlyph;
											isColorStale = true;
										}
					...
					public void UpdateGlyphBoards()
					{
						...
						else if (isColorStale)
						{
							...
							{
								FormattedGlyph fGlyph = formattedGlyphs[i];
								BoundedQuadBoard glyphBoard = glyphBoards[i];
								Vector4 newColor = fGlyph.format.Color.GetBbColor();

								if (Vector4.DistanceSquared(newColor, glyphBoard.quadBoard.materialData.bbColor) > .001f)
								{
									glyphBoard.quadBoard.materialData.bbColor = fGlyph.format.Color.GetBbColor();
									glyphBoards[i] = glyphBoard;
									isColorEqual = false;
								}
							}

							// The color may have changed, but by the time it was ready to draw, it changed back.
							if (isColorEqual)
								isColorStale = false;
